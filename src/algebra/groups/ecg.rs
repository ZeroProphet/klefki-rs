...
pub trait Curve<F, G> {
    fn new(x: prime::PrimeField<F>, y: prime::PrimeField<F>) -> Box<dyn EllipticCurveGroupProperty<F, G>>;
    fn add(a: CurvePoint<F>, b: CurvePoint<F>) -> Box<dyn EllipticCurveGroupProperty<F, G>>;
}

pub trait EllipticCurveGroupProperty<F, G>
where G: Curve<F, G>, F: prime::New
{
...
{
    fn zero() -> Self {
        return G::new(prime::PrimeField::<F>::zero(), prime::PrimeField::<F>::zero());
    }
...
where G: Curve<F, G>, F: prime::New
{
    fn eq(&self, rhs: &Self) -> bool {
        return self.point().x == rhs.point().x &&
